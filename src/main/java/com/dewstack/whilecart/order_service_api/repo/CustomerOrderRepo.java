...
public interface CustomerOrderRepo extends JpaRepository<CustomerOrder,String> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM customer_order WHERE remark=%?1%")
    public Page<CustomerOrder> searchAll(String remark, Pageable pageable);

    @Query(nativeQuery = true, value = "SELECT COUNT(order_id) FROM customer_order WHERE remark=%?1%")
